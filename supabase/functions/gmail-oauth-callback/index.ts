...
Deno.serve(async (req) => {
  ...
  const respond = (title: string, msg: string, ok: boolean, status = 200, service: string | null = null) =>
    new Response(
      oauthCallbackPage({ title, message: msg, ok, source: "nazai-gmail-oauth", redirectOrigin, extra: { service } }),
  ...
  const { data: txRows, error: txErr } = await admin.rpc("consume_gmail_oauth_transaction", { _state: state });
  ...
  try {
    ...
    return respond("Google connected", `Connected as ${info?.email || "Google account"}.`, true, 200, serviceKind);
  } catch (e) {
