...
serve(async (req) => {
  ...
  try {
    ...
    // Deduct credit atomically using the RPC (checks balance + deducts in one step)
    const { data: credited, error: creditError } = await supabaseClient.rpc('deduct_credit', { user_id: user.id })
