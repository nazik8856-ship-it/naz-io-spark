// youtube-oauth-callback — Google redirects here after the user grants
// YouTube consent. We persist tokens as provider "YouTube" (separate row
// from Gmail/Google Workspace) so both connections coexist per user.
...
try {
  if (window.opener) {
    window.opener.postMessage({ source:"nazai-youtube-oauth", ok:${ok}, message:${JSON.stringify(msg)} }, "*");
  }
} catch(e){}
...
Deno.serve(async (req) => {
  ...
  if (errParam) return respond("YouTube connection cancelled", errParam, false, 400);
  ...
  try {
    const tok = await exchangeGoogleCode(code, YOUTUBE_REDIRECT_URI);
    ...
    const credentials = {
      ...
      account_name: info?.name || info?.email || "YouTube",
      handle: info?.email || "YouTube",
    ...
    if (secretId) {
      ...
      secretId = await createSecret(admin, credentials, `youtube-${userId}-${agentId ?? "global"}`);
    }
    ...
    return respond("YouTube connected", `Connected as ${info?.email || "YouTube account"}. You can close this window.`, true);
  } catch (e) {
    return respond("YouTube connection failed", e instanceof Error ? e.message : "Unknown error", false, 500);
  }
});
