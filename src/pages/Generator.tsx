      ...
      <main className="flex-1 flex flex-col min-w-0 bg-[#020617] relative">
        ...
        <div className="flex-1 overflow-y-auto p-8 font-mono scrollbar-hide">
          <div className="max-w-4xl mx-auto space-y-6">
            ...
            {generatedCode ? (
              <div className="rounded-xl border border-white/10 overflow-hidden shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-700 bg-black/20">
                ...
                <div className="bg-[#010d1f] border-t border-white/5 px-6 py-4 flex items-center justify-between gap-4">
                  <div className="flex gap-3">
                    ...
                    {/* Integrated Save Button - Only shows when code is ready */}
                    <button 
                      onClick={handleSaveMission}
                      disabled={saveState === "saving" || saveState === "success"}
                      className="flex items-center gap-2 px-4 py-2 rounded-lg border transition-all text-[10px] font-mono uppercase"
                      style={{
                        borderColor: saveState === "success" ? "#22c55e" : saveState === "error" ? "#ef4444" : "rgba(16,185,129,0.4)",
                        background: saveState === "success" ? "rgba(34,197,94,0.1)" : "rgba(16,185,129,0.05)",
                        color: saveState === "success" ? "#22c55e" : saveState === "error" ? "#ef4444" : "#10b981",
                      }}
                    >
                      {saveState === "saving" ? <Loader2 className="animate-spin w-3.5 h-3.5" /> :
                       saveState === "success" ? <CheckCircle2 className="w-3.5 h-3.5" /> :
                       <DatabaseZap className="w-3.5 h-3.5" />}
                      {saveState === "saving" ? "Syncing..." : saveState === "success" ? "Archived" : "Save to DB"}
                    </button>
                  </div>

                  <div className="flex items-center gap-2 text-[10px] font-mono text-slate-500">
                    <ChevronRight className="w-3 h-3 text-emerald-600" />
                    <span>MISSION_ACQUIRED</span>
                  </div>
                </div>
              </div>
            ) : (
              <div className="h-[400px] flex flex-col items-center justify-center border-2 border-dashed border-white/5 rounded-3xl text-slate-600">
                <Zap className="w-12 h-12 mb-4 opacity-10" />
                <p className="text-sm font-mono tracking-widest uppercase opacity-40">Awaiting Directive...</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="p-8 bg-gradient-to-t from-[#020617] via-[#020617] to-transparent z-40">
          <div className={`max-w-4xl mx-auto rounded-2xl border transition-all duration-500 flex items-center px-4 py-1 ${focused ? "border-blue-500/40 bg-blue-500/5 shadow-[0_0_50px_rgba(0,163,255,0.05)]" : "border-white/10 bg-white/[0.02]"}`}>
            ...
            />
            {loading ? <div className="w-5 h-5 border-2 border-blue-500/20 border-t-blue-500 animate-spin rounded-full mr-2" /> : (
              <button onClick={handleGenerate} className="p-2 text-blue-500 hover:scale-110 active:scale-95 transition-all"><Zap className="w-5 h-5 fill-current" /></button>
            )}
          </div>
        </div>
      </main>
