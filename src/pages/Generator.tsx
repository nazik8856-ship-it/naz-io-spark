...
const Generator = () => {
  ...
  // DEBUG LOG: Watch this in your browser F12 Console
  useEffect(() => {
    console.log("NazAI OS - Code Length:", generatedCode.length);
  }, [generatedCode]);
  ...
  return (
    <div className="flex h-screen w-full bg-[#020617] text-slate-200 font-sans overflow-hidden relative">
      ...
      {/* ── DYNAMIC ARCHIVE BUTTON (FORCE RENDER) ── */}
      {generatedCode.length > 0 && !loading && (
        <div className="fixed bottom-32 right-10 z-[1000] animate-in fade-in zoom-in duration-500">
          <button 
            onClick={handleSaveMission}
            disabled={saveState !== "idle"}
            className="flex items-center gap-3 px-8 py-4 rounded-xl border-2 border-emerald-500 bg-[#061a11] text-emerald-400 text-sm font-black uppercase tracking-widest shadow-[0_0_40px_rgba(16,185,129,0.3)] hover:scale-105 active:scale-95 transition-all"
          >
            {saveState === "saving" ? <Loader2 className="animate-spin w-5 h-5" /> : <DatabaseZap className="w-5 h-5" />}
            {saveState === "success" ? "ARCHIVED" : "SAVE TO NAZAI_DATABASE"}
          </button>
        </div>
      )}
            ...
            <div className="rounded-3xl border border-white/10 bg-black/60 p-12 min-h-[600px] shadow-2xl backdrop-blur-xl">
              {loading ? (
                <div className="flex flex-col items-center justify-center h-[500px]">
                  ...
                  <p className="text-[10px] uppercase tracking-[0.5em] text-blue-400">Decrypting Response...</p>
                </div>
              ) : generatedCode ? (
                <div className="animate-in fade-in slide-in-from-bottom-8 duration-700">
                  <div className="flex items-center gap-2 mb-8 border-b border-white/5 pb-4">
                    <Sparkles className="w-5 h-5 text-emerald-400" />
                    <span className="text-xs text-emerald-400 uppercase font-black tracking-widest">Data_Unlocked // Solution_Ready</span>
                  </div>
                ...
                <div className="flex flex-col items-center justify-center h-[500px] opacity-10">
                  ...
                  <p className="text-xs uppercase tracking-[1em]">Awaiting Uplink</p>
                </div>
              )}
            </div>
          </div>
